    ...
    <div className={style.background}>
      <div className={style.wrapper}>
        <div className={style.alertWrapper}>
          {
            infoUserLog.state === 'Unverified' && infoUserLog.typeOfPerson === 'provider' &&
            <div className={style.verificationAlert}>
              <p className={style.textAlert}>
                Completa tu perfil para poder verificar tu cuenta. Debes tener los siguientes campos completos: Información personal, Foto de Perfil y Servicios. <br /> Ten en cuenta
          ...
          <div className={style.imageWrapper}>
            {
              infoUserLog.state != "Unverified" && infoUserLog.typeOfPerson === 'provider' ?
                infoUserLog.state === "Active"
                  ? <button type="button" onClick={() => handleChangeStatus("Inactive")} className={style.stateActive}>{infoUserLog.state}</button>
                  : <button type="button" onClick={() => handleChangeStatus("Active")} className={style.stateInactive}>{infoUserLog.state}</button>
                : <></>
            }
            <img
              ...
              alt="Imagen"
            />
            <UploadWidget setPublicId={setPublicId} user={infoUserLog.idPeople} />
            <div className={style.valoration}>
              {[...Array(Math.floor(Number(infoUserLog.averageRating)))].map((_, index) => (
          ...
          <div className={style.infoWrapper}>
            <div className={style.nameWrapper}>
              <p className={style.textName}>{infoUserLog.fullName}</p>
              {
                ...
                <div className={infoUserLog.state === "Unverified" ? style.iconNotVerified : style.iconVerified}></div>
              }
              {
                infoUserLog.typeOfPerson === 'provider'
