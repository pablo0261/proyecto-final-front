...
import video1 from '../../assets/Media/client-01.mp4';
...
const SlideCustomer = () => {
  ...
  const toggleVideoSize = () => {
    setIsVideoExpanded(prevState => !prevState);
  };
  ...
  useEffect(() => {
    // Si la sección del video está visible, reproducir automáticamente el video
    if (step1Visible) {
      videoRef.current.play();
    } else {
      // Pausar el video si la sección no está visible
  ...
  return (
    <div className={styles.container}>
      ...
      <section className={`${styles.section} ${step1Visible ? styles.sectionVisible : styles.sectionInVisible}`}>
        ...
        <video width="500" ref={videoRef} autoPlay loop
